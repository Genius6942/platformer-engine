"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.loadImage = void 0;
function loadImage(name, url, onProgress) {
    if (onProgress === void 0) { onProgress = function () { }; }
    return new Promise(function (resolve, reject) {
        var img = new Image();
        // img.addEventListener('load', () => {
        // 	resolve({name: name, img: img});
        // });
        // img.addEventListener('error', (e) => reject(e));
        // img.src = url;
        var xhr = new XMLHttpRequest();
        xhr.open("GET", url, true);
        xhr.responseType = "arraybuffer";
        xhr.addEventListener("load", function () {
            if (xhr.status === 200) {
                var blob = new Blob([xhr.response]);
                img.src = URL.createObjectURL(blob);
                resolve({ name: name, img: img });
            }
        });
        xhr.addEventListener("progress", function (e) {
            if (e.lengthComputable) {
                onProgress(e.loaded, e.total);
            }
            else {
                // average image size
                onProgress(0, 8.1 * 1000); // https://www.pingdom.com/blog/new-facts-and-figures-about-image-format-use-on-websites/#:~:text=Average%20image%20size%20per%20format,a%20Web%20page%20on%20average.
            }
        });
        xhr.addEventListener("error", function () {
            reject("Network error or blocked by cors");
        });
        xhr.send();
    });
}
exports.loadImage = loadImage;
function loadImages(images, onProgress) {
    if (onProgress === void 0) { onProgress = function () { }; }
    return __awaiter(this, void 0, void 0, function () {
        var loadedNumbers, totalNumbers, imgs, loadedImgs, finishedImgs;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.group("loading progress");
                    loadedNumbers = Object.keys(images).map(function () { return 0; });
                    totalNumbers = Object.keys(images).map(function () { return 1; });
                    imgs = Object.keys(images).map(function (key, idx) {
                        return loadImage(key, images[key], function (loaded, total) {
                            // console.log(key, (loaded / total) * 100);
                            loadedNumbers[idx] = loaded;
                            totalNumbers[idx] = total;
                            onProgress(loadedNumbers.reduce(function (a, b) { return a + b; }), totalNumbers.reduce(function (a, b) { return a + b; }));
                        });
                    });
                    return [4 /*yield*/, Promise.all(imgs)];
                case 1:
                    loadedImgs = _a.sent();
                    console.groupEnd();
                    finishedImgs = {};
                    loadedImgs.forEach(function (_a) {
                        var name = _a.name, img = _a.img;
                        finishedImgs[name] = img;
                    });
                    return [2 /*return*/, finishedImgs];
            }
        });
    });
}
exports["default"] = loadImages;
